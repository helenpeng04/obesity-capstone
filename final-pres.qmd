...
pretty_labels <- c(
  ...
  "percent_below_18_years_of_age" = "Below 18 Years Old",
...
beta_full <- betareg(obesity_prop ~ percent_physically_inactive + median_household_income + 
                      ...
                      percent_uninsured_adults + traffic_volume + food_environment_index, 
                    data = data_2025)
...
plot(beta_simple, which = 1:4,
     ...
     main = "") # does not look like it pass the tests ahhh

```

```{r beta_full_diagnostics, eval = FALSE}
plot(beta_full, which = 1:4,
     caption = c("Residuals vs indices of obs.", "Cook's distance plot",
                 "Generalized leverage vs predicted values", "Residuals vs linear predictor"),
     main = "") # multicollinearity issues
...
margin_df |> 
  ...
  kable(col.names = c("Predictor", "Model", "Effect (%)", "Lower CI", "Upper CI", "P-value"), digits = 2, format = "html") |>
...
-   Add individual factors relationship with obesity rate scatter plots maybe

-   Beta regression diagnostic plots
