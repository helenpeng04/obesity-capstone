...
# data_2025 <- na.omit(data_2025) no need to do this for XGBoost
features <- data_2025[, !(names(data_2025) %in% c("percent_adults_with_obesity", "fips", "state", "county"))]
label <- data_2025$percent_adults_with_obesity

data_matrix <- as.matrix(features)
...
pretty_labels <- c(
  ...
  "percent_feeling_lonely" = "Feeling Lonely",
  "percent_children_in_single_parent_households" = "Children in Single Parent Household",
...
# A 1% increase in the percentage of physically inactive adults in a county is associated with a 3.1% increase in the odds that an adult in that county has obesity.
# 
# A one unit increase in the food environment index is associated with a 1.7% increase in the odds that an adult in that county has obesity.
```

```{r multivariate_model}
# We selected variables based on correlation analysis, theoretical relevance, and robustness to outliers, avoiding machine learning-based selection methods to preserve interpretability.
```
