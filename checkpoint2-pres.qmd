...
statewide_physical_inactivity <- data_2025 |>
  filter(is.na(county) & !is.na(percent_physically_inactive)) |>
  select(state, percent_physically_inactive) |>
  rename(statewide_physical_inactivity = percent_physically_inactive)

data_2025 <- data_2025 |> 
  left_join(statewide_physical_inactivity, by = "state") |> 
  mutate(
    percent_physically_inactive = if_else(
      is.na(percent_physically_inactive) & !is.na(county),
      statewide_physical_inactivity,
      percent_physically_inactive
    )
  ) |> 
  select(-statewide_physical_inactivity) |> 
  filter(!is.na(county))

updated_missing_data <- data_2025 |> 
 filter((is.na(percent_adults_with_obesity) |
          is.na(percent_physically_inactive) |
          is.na(food_environment_index)) &
         !is.na(fips) &
