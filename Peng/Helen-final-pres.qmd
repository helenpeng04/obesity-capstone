...
# 1. Remove rows with missing outcome
data_clean <- data_2025 %>% filter(!is.na(percent_adults_with_obesity))

# 2. Remove unwanted columns including obesity_prop and logit_obesity
X_df <- data_clean %>%
  dplyr::select(-fips, -state, -county, -percent_adults_with_obesity, -obesity_prop, -logit_obesity, -cluster)

# 3. Convert factors and characters to numeric
...
# 4. Convert to numeric matrix for xgboost
X <- as.matrix(X_df)

# 5. Define label vector
y <- data_clean$percent_adults_with_obesity
...
# 7. Get top 15 variable importance scores
top_vi <- vi(xgb_model) %>% dplyr::slice_max(Importance, n = 15)

# 8. Rename variables for nicer labels
pretty_labels <- c(
...
vip(top_vi, aesthetics = list(fill = "steelblue")) +
  labs(y = "Variable Importance (Gain)", title = "Top 15 XGBoost Variable Importance")
...
summary(rlm(logit_obesity ~ percent_physically_inactive + percent_limited_access_to_healthy_foods + median_household_income, data = data_2025))
